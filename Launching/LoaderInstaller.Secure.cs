...
/// </summary>
public sealed class SecureLoaderInstaller
{
    ...
    private static readonly TimeSpan InstallStallTimeout = TimeSpan.FromMinutes(3);
    ...
    public SecureLoaderInstaller(MinecraftPath path, HttpClient http, Action<string>? log = null)
    {
        ...
        CancellationToken ct)
    {
        ...
        if (compatibilityUrl.Length > 0 && !distribution.InstallerMirrors.Contains(compatibilityUrl, StringComparer.OrdinalIgnoreCase))
        ...
        try
        {
            ...
            var installed = await InstallOriginalJarAsync(installerPath, loaderVersion, expectedId, distribution, ct).ConfigureAwait(false);
    ...
    private async Task DownloadAsync(Uri uri, string path, CancellationToken ct)
    {
        ...
        linked.CancelAfter(uri.Host.Equals("maven.neoforged.net", StringComparison.OrdinalIgnoreCase) ? OfficialDownloadTimeout : DownloadTimeout);
        ...
        await using var output = new FileStream(path, FileMode.Create, FileAccess.Write, FileShare.None, 128 * 1024, FileOptions.Asynchronous | FileOptions.SequentialScan);
        byte[]? buffer = null;
        try
        {
            ...
            while (true)
            {
                ...
                if (read <= 0) break;
                total += read;
                if (total > MaxInstallerBytes) throw new InvalidOperationException("NeoForge installer превышает безопасный размер.");
    ...
    private static void RequireInstallerHash(string path, string expected)
    {
        ...
        if (!NeoForgeDistributionBootstrap.IsSha256(expected)) throw new InvalidOperationException("Invalid expected installer SHA-256.");
    ...
    private static void ValidateInstallerMetadata(string path, string loaderVersion)
    {
                     ...
                     entry.Length > 0 && entry.Length <= MaxMetadataEntryBytes))
        {
            ...
            if (text.Contains("neoforge", StringComparison.OrdinalIgnoreCase) && text.Contains(loaderVersion, StringComparison.OrdinalIgnoreCase)) return;
        }
        ...
        foreach (var mirror in NeoForgeDistributionBootstrap.GetEffectiveMavenMirrors(distribution))
        {
            ...
            try
            {
                ...
                var result = await RunJavaAsync(
                    java,
                    new[] { "-jar", installerPath, "--installClient", distribution.InstallerMirrorArgument, normalized },
                    root,
                    env,
                ...
                if (installed is null) throw new InvalidOperationException("Installer did not create NeoForge version JSON.");
    ...
    private async Task<string> FindJavaAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (Directory.Exists(runtime)) candidates.AddRange(Directory.EnumerateFiles(runtime, "java.exe", SearchOption.AllDirectories));
        }
        catch { }
        ...
        foreach (var candidate in candidates.Distinct(StringComparer.OrdinalIgnoreCase))
        {
            ...
            if (probe is { Major: >= 21, Is64Bit: true }) return candidate;
        }
    ...
    private static async Task<JavaProbe?> ProbeJavaAsync(string java, CancellationToken ct)
    {
        try
        {
            ...
            var psi = new ProcessStartInfo(java) { UseShellExecute = false, RedirectStandardOutput = true, RedirectStandardError = true, CreateNoWindow = true };
    ...
    private static int ParseJavaMajor(string text)
    {
        foreach (var line in text.Split('\n'))
        {
            ...
            {
                var token = value["java.version =".Length..].Trim().Split('.', '-', '+')[0];
                if (int.TryParse(token, out var major)) return major;
            }
        }
        return 0;
    }
        ...
        CancellationToken ct)
    {
        ...
        var psi = new ProcessStartInfo(java) { WorkingDirectory = workingDirectory, UseShellExecute = false, RedirectStandardOutput = true, RedirectStandardError = true, CreateNoWindow = true };
    ...
    private static string? FindNeoForgeVersion(string root, string loaderVersion)
    {
        ...
        return Directory.EnumerateDirectories(versions).Select(Path.GetFileName)
            .Where(static value => !string.IsNullOrWhiteSpace(value)).Select(static value => value!)
            .FirstOrDefault(value => value.Contains("neoforge", StringComparison.OrdinalIgnoreCase) && value.Contains(loaderVersion, StringComparison.OrdinalIgnoreCase));
    }

    private static void WriteLauncherProfile(string mcDir)
    {
        var json = JsonSerializer.Serialize(new { profiles = new Dictionary<string, object>(), settings = new Dictionary<string, object>(), launcherVersion = new { name = "LegendBorn", format = 21 } });
