...
public sealed class LoaderInstaller
{
        ...
        CancellationToken ct)
    {
        ...
        if (loaderType != "neoforge")
            throw new NotSupportedException($"Loader '{loaderType}' не поддерживается.");
        ...
        if (!NeoForgeDistributionBootstrap.TryResolve(loaderVersion, out var distribution))
            throw new InvalidOperationException(
                $"Для NeoForge {loaderVersion} отсутствует доверенный distribution contract.");
        ...
        {
            throw new InvalidOperationException(
                "loader.installerUrl не входит в catalog installerMirrors.");
        }
        ...
        try
        {
            ...
            var installerPath = await DownloadVerifiedInstallerAsync(
                    minecraftVersion,
                    distribution,
                    ct)
            ...
            var installed = await InstallOriginalJarAsync(
                    installerPath,
                    minecraftVersion,
                    loaderVersion,
                    expectedId,
            ...
            return installed ??
                   throw new InvalidOperationException(
                       "NeoForge installer завершился, но version JSON не найден.");
        }
        finally
        {
            gate.Release();
        ...
        CancellationToken ct)
    {
        ...
        var dir = Path.Combine(
            ...
            "neoforge",
            minecraftVersion,
            distribution.LoaderVersion);
        Directory.CreateDirectory(dir);

        var finalPath = Path.Combine(
            dir,
            $"neoforge-{distribution.LoaderVersion}-installer.jar");
        ...
        if (File.Exists(finalPath))
        {
            try
            {
                ...
                _log?.Invoke("NeoForge installer: использую проверенный локальный кеш.");
                return finalPath;
            }
            catch
            {
        ...
        Exception? last = null;
        foreach (var candidate in distribution.InstallerMirrors
                     .Distinct(StringComparer.OrdinalIgnoreCase))
        {
            ...
            try
            {
                ...
                if (source == "BMCLAPI")
                    _log?.Invoke("Источник загрузки: BMCLAPI");
        ...
        throw new InvalidOperationException(
            "Ни одно installer mirror не отдало JAR с ожидаемым SHA-256.",
            last);
    }
        ...
        CancellationToken ct)
    {
        ...
        if (response.Content.Headers.ContentLength is long contentLength)
        {
            if (contentLength <= 0 || contentLength > maxBytes)
                throw new InvalidOperationException($"Некорректный размер файла: {contentLength}");
            if (expectedSize is > 0 && contentLength != expectedSize.Value)
                throw new InvalidOperationException(
                    $"Размер файла не совпал: expected={expectedSize.Value}, actual={contentLength}");
        }

        var mediaType = response.Content.Headers.ContentType?.MediaType ?? "";
        if (mediaType.Contains("text/html", StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException("Вместо бинарного файла получена HTML-страница.");
        ...
        try
        {
            ...
            while (true)
            {
                ...
                if (total > maxBytes)
                    throw new InvalidOperationException(
                        $"Файл превышает безопасный размер {maxBytes} bytes.");
            ...
            if (expectedSize is > 0 && total != expectedSize.Value)
                throw new InvalidOperationException(
                    $"Размер файла не совпал: expected={expectedSize.Value}, actual={total}");
    ...
    private static void RequireInstallerHash(string path, string expected)
    {
        ...
        {
            throw new InvalidOperationException(
                $"NeoForge SHA-256 mismatch: expected={expected}, actual={actual}");
        }
    }
    ...
    private static void ValidateInstallerMetadata(string path, string loaderVersion)
    {
        ...
        foreach (var entry in zip.Entries.Where(static entry =>
                     (entry.FullName.EndsWith(
                      ...
                      entry.FullName.EndsWith(
                          ...
                          StringComparison.OrdinalIgnoreCase)) &&
                     entry.Length > 0 &&
                     entry.Length <= MaxMetadataEntryBytes))
        {
        ...
        if (!matched)
            throw new InvalidOperationException(
                "NeoForge installer metadata does not match requested version.");
    }
        ...
        CancellationToken ct)
    {
        ...
        foreach (var mirror in NeoForgeDistributionBootstrap.GetEffectiveMavenMirrors(distribution))
        {
            ...
            if (source == "BMCLAPI")
                _log?.Invoke("Источник зависимостей: BMCLAPI");
            ...
            try
            {
                ...
                SeedVanillaVersion(root, tempMc, minecraftVersion);

                await PrefetchNeoForgeMavenArtifactsAsync(
                        installerPath,
                        tempMc,
                        normalized,
                        ct)
                ...
                if (installed is null)
                    throw new InvalidOperationException(
                        "Installer did not create NeoForge version JSON.");
        ...
        throw new InvalidOperationException(
            "NeoForge installation failed on all Maven mirrors.",
            last);
    }

    private async Task PrefetchNeoForgeMavenArtifactsAsync(
        string installerPath,
        string tempMc,
        string mirrorBase,
        CancellationToken ct)
    {
        var artifacts = ReadNeoForgeHostedArtifacts(installerPath);
        if (artifacts.Count == 0)
        {
            _log?.Invoke("NeoForge Maven prefetch: в installer metadata нет внешних NeoForge artifacts.");
            return;
        }

        var librariesRoot = Path.Combine(tempMc, "libraries");
        Directory.CreateDirectory(librariesRoot);
        var rootFull = Path.GetFullPath(librariesRoot)
            .TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar) +
            Path.DirectorySeparatorChar;
        var baseUri = new Uri(mirrorBase, UriKind.Absolute);
        ...
        foreach (var artifact in artifacts)
        {
            ...
            var relativePath = NormalizeMavenPath(artifact.Path);
            if (relativePath.Length == 0)
                throw new InvalidOperationException(
                    $"Некорректный Maven path в installer metadata: {artifact.Path}");
            ...
            if (!target.StartsWith(rootFull, StringComparison.OrdinalIgnoreCase))
                throw new InvalidOperationException("Maven artifact path вышел за libraries sandbox.");

            if (File.Exists(target) && VerifySha1(target, artifact.Sha1, artifact.Size))
                continue;
            ...
            try
            {
                var sourceUri = new Uri(baseUri, relativePath);
                await DownloadFileAsync(
                        sourceUri,
                        temp,
                        MaxLibraryArtifactBytes,
                        artifact.Size > 0 ? artifact.Size : null,
                        ct)
                    .ConfigureAwait(false);

                if (!VerifySha1(temp, artifact.Sha1, artifact.Size))
                {
                    throw new InvalidOperationException(
                        $"SHA-1 mismatch для {relativePath}");
                }
        ...
        _log?.Invoke(
            $"NeoForge Maven prefetch: проверено {artifacts.Count}, загружено {downloaded} artifact(s).");
    }

    private static IReadOnlyList<InstallerArtifact> ReadNeoForgeHostedArtifacts(string installerPath)
    {
        var result = new Dictionary<string, InstallerArtifact>(StringComparer.OrdinalIgnoreCase);
        ...
        foreach (var entryName in new[] { "install_profile.json", "version.json" })
        {
            var entry = zip.Entries.FirstOrDefault(entry =>
                ...
                entry.FullName.EndsWith('/' + entryName, StringComparison.OrdinalIgnoreCase));
            if (entry is null || entry.Length <= 0 || entry.Length > MaxMetadataEntryBytes)
                continue;
            ...
            {
                if (!TryReadArtifact(library, out var artifact)) continue;
                if (!Uri.TryCreate(artifact.Url, UriKind.Absolute, out var originalUri) ||
                    originalUri.Scheme != Uri.UriSchemeHttps ||
                    !originalUri.Host.Equals(
                        "maven.neoforged.net",
                        StringComparison.OrdinalIgnoreCase))
                    continue;

                var path = NormalizeMavenPath(artifact.Path);
                if (path.Length == 0 || !IsSha1(artifact.Sha1))
                    throw new InvalidOperationException(
                        "NeoForge installer содержит Maven artifact без безопасного path/SHA-1.");
                ...
                if (result.TryGetValue(path, out var existing) &&
                    (!string.Equals(existing.Sha1, artifact.Sha1, StringComparison.Ordinal) ||
                     (existing.Size > 0 && artifact.Size > 0 && existing.Size != artifact.Size)))
                {
                    throw new InvalidOperationException(
                        $"Installer metadata содержит конфликтующие hashes для {path}.");
                }
    ...
    private static bool TryReadArtifact(JsonElement library, out InstallerArtifact artifact)
    {
        artifact = new InstallerArtifact("", "", 0, "");
        if (library.ValueKind != JsonValueKind.Object ||
            ...
            !downloads.TryGetProperty("artifact", out var node) ||
            node.ValueKind != JsonValueKind.Object)
            return false;

        var path = GetJsonString(node, "path");
        var sha1 = GetJsonString(node, "sha1");
        var url = GetJsonString(node, "url");
        long size = 0;
        if (node.TryGetProperty("size", out var sizeNode) &&
            sizeNode.ValueKind == JsonValueKind.Number)
            sizeNode.TryGetInt64(out size);

        if (path.Length == 0 || url.Length == 0) return false;
        artifact = new InstallerArtifact(path, sha1, size, url);
        return true;
    }

    private static string GetJsonString(JsonElement node, string propertyName)
    {
    ...
    private static string NormalizeMavenPath(string? value)
    {
    ...
    private static bool VerifySha1(string path, string expectedSha1, long expectedSize)
    {
        if (!File.Exists(path) || !IsSha1(expectedSha1)) return false;
        try
        {
            var info = new FileInfo(path);
            if (expectedSize > 0 && info.Length != expectedSize) return false;
            using var stream = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.Read);
            var actual = Convert.ToHexString(SHA1.HashData(stream)).ToLowerInvariant();
            return string.Equals(
                actual,
                expectedSha1.Trim().ToLowerInvariant(),
    ...
    private static void SeedVanillaVersion(
        string root,
        string tempMc,
        string minecraftVersion)
    {
        var source = Path.Combine(root, "versions", minecraftVersion);
        if (!Directory.Exists(source)) return;
        var destination = Path.Combine(tempMc, "versions", minecraftVersion);
        MergeDir(source, destination);
    }
    ...
    private async Task<string> FindJavaAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (Directory.Exists(runtime))
                candidates.AddRange(
                    Directory.EnumerateFiles(runtime, "java.exe", SearchOption.AllDirectories));
        }
        catch
        {
        }
        ...
        foreach (var line in text.Split('\n'))
        {
            ...
            if (value.StartsWith("java.version =", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (int.TryParse(token, out var major))
                {
                    ...
                    if (parts.Length > 1 && int.TryParse(parts[1], out var legacy))
                        return legacy;
                }
            }
            ...
            if (marker >= 0)
            {
                ...
                if (end > start)
                {
                    var version = value[start..end];
                    var token = version.Split('.', '-', '+')[0];
                    if (int.TryParse(token, out var quotedMajor))
                    {
                        if (quotedMajor != 1) return quotedMajor;
                        var parts = version.Split('.');
                        if (parts.Length > 1 && int.TryParse(parts[1], out var legacy))
                            return legacy;
                    }
        ...
        CancellationToken ct)
    {
        ...
        catch (OperationCanceledException)
        {
            ...
            throw new TimeoutException(
                $"NeoForge installer timeout after {InstallOverallTimeout}.");
        }
    ...
    private static void WriteLauncherProfile(string mcDir)
    {
        ...
        File.WriteAllText(
            Path.Combine(mcDir, "launcher_profiles_microsoft_store.json"),
            json);
    }
