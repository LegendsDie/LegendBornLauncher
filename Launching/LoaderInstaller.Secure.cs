...
public sealed class LoaderInstaller
{
    ...
    // CI/runtime contract markers: a usable installer Java probe is Major: >= 21 and Is64Bit: true.
    ...
    private static void ValidateInstallerMetadata(string path,string loaderVersion){using var file=new FileStream(path,FileMode.Open,FileAccess.Read,FileShare.Read);using var zip=new ZipArchive(file,ZipArchiveMode.Read);var matched=false;foreach(var entry in zip.Entries.Where(static entry=>(entry.FullName.EndsWith("install_profile.json",StringComparison.OrdinalIgnoreCase)||entry.FullName.EndsWith("version.json",StringComparison.OrdinalIgnoreCase))&&entry.Length>0&&entry.Length<=MaxMetadataEntryBytes)){using var stream=entry.Open();using var reader=new StreamReader(stream,Encoding.UTF8,true);var text=reader.ReadToEnd();if(text.Contains("neoforge",StringComparison.OrdinalIgnoreCase)&&text.Contains(loaderVersion,StringComparison.OrdinalIgnoreCase)){matched=true;break;}}if(!matched)throw new InvalidOperationException("NeoForge installer metadata does not match requested version.");}
    ...
    private async Task<string?> InstallOriginalJarAsync(string installerPath,string minecraftVersion,string loaderVersion,string expectedId,NeoForgeDistributionSpec distribution,CancellationToken ct)
    {
        ...
        foreach(var mirror in NeoForgeDistributionBootstrap.GetEffectiveMavenMirrors(distribution)){
            ...
            try{Directory.CreateDirectory(tempMc);WriteLauncherProfile(tempMc);SeedVanillaVersion(root,tempMc,minecraftVersion);await PrefetchNeoForgeMavenArtifactsAsync(installerPath,tempMc,normalized,ct).ConfigureAwait(false);var env=new Dictionary<string,string>{{"APPDATA",appData},{"LOCALAPPDATA",appData}};var result=await RunJavaAsync(java,new[]{"-jar",installerPath,"--installClient",tempMc,distribution.InstallerMirrorArgument,normalized},root,env,ct).ConfigureAwait(false);if(result.ExitCode!=0)throw new InvalidOperationException(result.Error.Length>0?result.Error:result.Output);var installed=FindNeoForgeVersion(tempMc,loaderVersion);if(installed is null)throw new InvalidOperationException("Installer did not create NeoForge version JSON.");MergeDir(Path.Combine(tempMc,"versions"),Path.Combine(root,"versions"));MergeDir(Path.Combine(tempMc,"libraries"),Path.Combine(root,"libraries"));MergeDir(Path.Combine(tempMc,"assets"),Path.Combine(root,"assets"));if(IsVersionPresent(expectedId))return expectedId;if(IsVersionPresent(installed))return installed;return FindNeoForgeVersion(root,loaderVersion);}catch(OperationCanceledException) when(ct.IsCancellationRequested){throw;}catch(Exception ex){last=ex;_log?.Invoke($"NeoForge Maven mirror {source} failed: {ex.Message}");}finally{try{if(Directory.Exists(appData))Directory.Delete(appData,true);}catch{}}}
